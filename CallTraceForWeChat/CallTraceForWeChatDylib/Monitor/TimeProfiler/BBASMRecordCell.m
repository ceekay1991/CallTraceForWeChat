...
@implementation BBASMRecordCell
...
- (void)bindRecordModel:(BBASMRecordModel *)model
    ...
    CGFloat time = model.costTime/1000.0;
    if (time > 10) {
        self.timeLabel.backgroundColor = [UIColor redColor];
    } else {
        self.timeLabel.backgroundColor = [UIColor whiteColor];
    }
    self.timeLabel.text = [NSString stringWithFormat:@"%lgms", time];
    ...
    if (model.isMainThread) {
        self.methodLabel.textColor = [UIColor blueColor];
    } else {
...
- (UIButton *)expandBtn {
    if (!_expandBtn) {
        ...
        _expandBtn.frame = CGRectMake(kDepthLabelWidth+kTimeLabelWidth+16, 0, 18, 18);
